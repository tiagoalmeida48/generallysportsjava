						...
						<tbody>
							<c:forEach items="${listaPaginada.content}" var="produto">
								<tr>
									...
									<td><c:if
											test="${produto.getCaminhoFoto() == null || produto.getCaminhoFoto() == '' }">
											<img src="/img/default_product.png" class="product-image"
												width=30 alt="Imagem do Produto">
										</c:if> <c:if
											test="${produto.getCaminhoFoto() != null && produto.getCaminhoFoto() != ''}">
											<img src="/img/${produto.getCaminhoFoto() }"
												class="product-image" width=30 alt="Imagem do Produto">
										</c:if></td>
									...
									<td class="center-align"><a
										...
										href="${s:mvcUrl('visualizarProduto').arg(0, produto.idProduto).build() }"
										title="Visualizar"><i
											class="material-icons yellow-text text-darken-3">remove_red_eye</i></a>
										<a class="waves-effect waves-light btn-small blue"
										href="${s:mvcUrl('editarProduto').arg(0, produto.idProduto).build() }"
										title="Editar"><i
										...
										<button href="#modalExcluir"
											...
											data-id="${produto.idProduto }" title="Excluir">
